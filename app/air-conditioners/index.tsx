...
function ConditionersList({ conditionerList, brands }: any) {
  ...
  return (
    <div className="conditioners container">
      <h2 className="text-center font-bold text-6xl text-slate-600">
        {brands}
      </h2>
      <ul
        ref={ref}
        className="conditioners__list grid grid-cols-3 gap-12 mt-14"
      >
        {conditionerList.map((el: any, index: number) => {
          return (
            <Link key={index} href={`/air-conditioners/${el.name}`}>
              <li
              ...
              >
                ...
                <div className="flex flex-col">
                  <p className="text-5xl font-medium relative z-10">
                    {el.name}
                  </p>
                ...
                <button className="conditioners__btn mt-10">
                  Выбрать параметры
                </button>
              </li>
            </Link>
